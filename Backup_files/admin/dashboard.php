...
// ✅ Query from recruitment DB (not from default $connections)
$stmt = $hr1_recruitment->prepare("SELECT status, COUNT(*) as count FROM jobs GROUP BY status");
if ($stmt) {
    $stmt->execute();
    ...
    die("Query preparation failed: " . $hr1_recruitment->error);
}
?>
